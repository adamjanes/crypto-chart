...
const App = () => {
  ...
  useEffect(() => {
    ...
    // grab as much data from coin gecko as possible untill we get rate limited
                ...
                vals: [
                  {
                    ...
                    price: -(+d.market_data?.price_change_percentage_60d_in_currency?.usd || 0),
                  },
                  {
                    date: new Date(new Date().setDate(new Date().getDate() - 30)),
                    price: -(+d.market_data?.price_change_percentage_30d_in_currency?.usd || 0),
                  },
                  {
                    date: new Date(new Date().setDate(new Date().getDate() - 14)),
                    price: -(+d.market_data?.price_change_percentage_14d_in_currency?.usd || 0),
                  },
                  {
                    date: new Date(new Date().setDate(new Date().getDate() - 7)),
                    price: -(+d.market_data?.price_change_percentage_7d_in_currency?.usd || 0),
                  },
                  {
                    date: new Date(new Date().setDate(new Date().getDate() - 1)),
                    price: -(+d.market_data?.price_change_percentage_24h_in_currency?.usd || 0),
                  },
                  {
                    date: new Date(),
                    price: 0.01,
      ...
      <Container style={{ height: '500px' }}>
        <Grid style={{ height: '500px' }} container spacing={3}>
          <Grid style={{ height: '500px' }} item xs={12} md={9}>
            {data.length !== 0 && <LineChart data={data} setSelected={setSelected} />}
          </Grid>
